...
match_results = match_results.sort_values(by=['game'], ascending=False)


form_btwn_teams = get_form_features(match_results)
features = get_base_feature_frame(match_results)
features = features.merge(form_btwn_teams, on=['game', 'home_team', 'away_team',
                                               'home_team_id', 'away_team_id', 'ground_id'])

# ELO
elos_fwd, probs_fwd, elo_dict_fwd = elo_getter.elo_applier(match_results, 30)

# # Add our created features - elo, efficiency etc.
features_with_elo = (features.assign(
    f_elo_home=lambda df: df.game.map(elos_fwd).apply(lambda x: x[0]),
    f_elo_away=lambda df: df.game.map(elos_fwd).apply(lambda x: x[1])))

features_with_elo = features_with_elo.merge(match_results[['game', 'home_score', 'away_score']],
                                            on="game",
                                            how="inner")

features_with_elo['f_home_ground_adv'] = features_with_elo['home_ground_adv'].apply(lambda x: 1.0 if True else 0.0)
features_with_elo['f_away_ground_adv'] = features_with_elo['away_ground_adv'].apply(lambda x: 1.0 if True else 0.0)
...
train_df = features_with_elo[features_with_elo.season == __year__]
feature_cols = []
feature_cols.extend(['game', 'f_away_team_id', 'f_home_team_id',
                     'f_ground_id', 'f_home_ground_adv', 'f_away_ground_adv'])

# Create our train set
X = features_with_elo[features_with_elo.season != __year__][feature_cols]
